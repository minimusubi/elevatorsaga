...
function Elevator(speedFloorsPerSec, floorCount, floorHeight, maxUsers) {
	...
	elevator.buttonStates = _.map(_.range(floorCount), (e, i) => {
		return false;
	});
	elevator.moveCount = 0;
	elevator.removed = false;
	elevator.userSlots = _.map(_.range(elevator.maxUsers), (user, i) => {
